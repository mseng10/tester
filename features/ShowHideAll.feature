Feature: Allow CardGame user to end a game
  ...
  Background: I select the Login/Sign up page
    ...
    And I create a game with "1" decks, "1" sinks, with "jokers" and "shuffled", of hand size "4"
    Then I should be on the lobby page
    When I select the start game button
    Then I should be on the started game page

  Scenario: Select show all button
    When I select the show all button
    Then All my cards should be shown

  Scenario: Select hide all button
    When I select the hide all button
    Then All my cards should be hidden